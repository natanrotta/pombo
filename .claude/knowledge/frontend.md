...
- [High] **`colorPalette` is inert on this app's button recipe.** `solid`/`subtle`/`outline`/`ghost` paint explicit brand tokens, so `colorPalette="red"` still renders green — a destructive CTA must use `variant="danger"` (cast `as "solid"` until typegen runs). The same symptom still lives on the device-detail "Excluir" outline button (Fase 7). Grep `colorPalette=` on `<Button>` before trusting it.
...
- [Medium] **jsdom computes no colors** (`getComputedStyle(...).backgroundColor` is always transparent). To pin a Chakra style in Vitest, read the rules emotion injected for the element's single `css-*` class from the `<style>` tags (`AppModal.spec.tsx` → `injectedCssFor`). Recipe styles sit inside `@layer recipes`; unlayered style props beat them, so a spread-props fix needs a layer-aware check — switching the `variant` keeps it simple.
- [Medium] **Gate dev-only code in the build, not by convention.** A Vite plugin's `generateBundle` can walk each chunk's `modules` map and `this.error()` on a forbidden path (`pombo:dev-only-modules-excluded` in `vite.config.ts`); probe it by removing the `import.meta.env.DEV` branch. CI does not run `build:web` until Fase 10.
