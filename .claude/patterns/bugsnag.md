...
- `autoDetectErrors` stays at its default (`true`) in both apps — process-level `uncaughtException`/`unhandledRejection` (BullMQ workers, cron, Baileys socket callbacks) and browser `window.onerror`/`unhandledrejection` are captured without a call site.
- `appVersion: env.APP_VERSION` (api — the git SHA/tag CI stamps into the image via `Dockerfile.prod`; the same value `GET /api/health` returns) and `releaseStage` from `NODE_ENV`/`MODE`. [`BS-M3`]
...
| API process-level uncaught / unhandled rejection (workers, cron, Baileys socket callbacks) | Bugsnag `autoDetectErrors` (default) | unhandled ⇒ `error` | yes (process-level) |
