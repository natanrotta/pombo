...
- **Secrets** — single validated config `core/config/env.ts` (Zod); **no scattered `process.env`**. Any dev-default secret must be a real value in prod. There is **no at-rest encryption primitive** in the tree today (the cuidda `AesGcmEncryptionService` was dead code and was removed) — the first provider-issued secret that must be persisted brings one in, in `core/service/`, behind a `shared/provider` port.
...
- **Outbound webhooks** — `modules/webhooks`: every event is signed with **HMAC-SHA256** over the raw body using the device's `webhook_secret` (`hmac-signer.ts`), delivered by `HttpWebhookSender` with a timeout (`WEBHOOK_TIMEOUT_MS`) and bounded retries (`WEBHOOK_MAX_ATTEMPTS`, exponential base `WEBHOOK_RETRY_BASE_DELAY_MS`), disconnect flaps debounced. The URL is **customer-supplied** (`UpdateDeviceWebhooksDTOSchema`: `url()` + `http(s)` only) — the sender is an SSRF surface by construction: the guard is `assertSafeOutboundUrl` (`shared/util/ssrf-guard.ts`): `http(s)` only, the host is resolved **at every attempt** right before the `fetch` (DNS rebinding), **every** record must be public (loopback, RFC 1918, link-local/cloud-metadata, CGNAT, ULA, multicast, unspecified, and every IPv4-embedding IPv6 form — `::ffff:`, the deprecated `::a.b.c.d`, the NAT64 `64:ff9b::/96` + `64:ff9b:1::/48` prefixes), `redirect: "manual"` and any 3xx is a final rejection. Never echo the response body to logs. Reuse the guard for any future outbound fetch (`SEC-C6`). Any **inbound** webhook you add must mount before the JSON parser with `express.raw` and verify a signature (`SEC-C3`).
...
| SEC-C6 | **Injection / SSRF** — raw SQL with string interpolation, shell exec with user input, a server fetch to a user-controlled URL that bypasses `assertSafeOutboundUrl` (scheme + private-range + IPv4-embedding-IPv6 forms + per-attempt resolution; the webhook sender is the live case), or unsafe deserialization | Classic RCE/injection (⊃ `B-C7`) |
...
| SEC-H8 | New secret added without env validation + rotation path, a dev-default secret left in prod, or a provider-issued secret persisted in clear | Register in `env.ts`; require a real value in prod (`X-H4`); encrypt at rest (bring the primitive in `core/service/` behind a port — none exists today) |
...
4. **Prefer existing primitives.** The repo ships the secure way (scoped repository signatures, `validateRequest`, the limiters, the JWT/bcrypt providers, the hashed API tokens, the SSRF guard, the redact list, the HMAC webhook signer). A security fix is usually "use the existing primitive", not "invent crypto".
