...
- [2026-09-16] 10d (parte 1, pendências de comportamento): as leituras dos repositórios recebem o `signal` do TanStack Query (um `cancelQueries` aborta o HTTP) e o `httpClient` repassa o cancelamento como tal, não como erro de rede; `useFormState.setField` com identidade estável (valida contra os valores mais recentes, inclusive dois `setField` no mesmo tick); o preview do avatar sai depois do envio, com sucesso ou erro, e o avatar volta à URL do usuário; o nome vazio no perfil mostra o motivo (antes o validador devolvia um código cru que ninguém exibia); `EntityCard` com `className="group"` e as ações rápidas também aparecem com foco de teclado — `.claude/commands/frontend.md` e `patterns/frontend.md` corrigidos. Cada item com teste que falha no código anterior. Adiado para o tema novo (handoff de design, agora disponível para importação): cores cruas restantes (`PasswordStrengthIndicator`, borda `brand.500` da sidebar, `white` do overlay do avatar) e o contraste das iniciais do avatar — a Fase 3 redefine esses tokens.
- [2026-09-16] 10d `/security` (limitadores de auth, token `email:verify` no `sessionStorage`, bypass do proxy do Vite): 0 Critical/High no código. High de infraestrutura, anterior à branch e já registrado em `.claude/knowledge/devops.md` como bloqueio para tráfego real: o firewall do host aceita 80/443 de qualquer origem, então quem chega direto no IP do servidor forja o `CF-Connecting-IP` e escolhe a chave de todos os limitadores por IP — fechar liberando só as faixas da Cloudflare (ou Authenticated Origin Pulls) antes de produção; fica fora deste PR. Mediums aceitos como risco residual: limitador só por IP (um bloqueio por conta abriria DoS contra a vítima) e códigos de erro que revelam conta existente/só Google. Lows: TTL de 1 h do token de verificação; o mesmo balde de limite em 9 rotas de auth. O override `RATE_LIMIT_AUTH_MAX=100` do e2e não alcança produção; o `bypass` do proxy só existe no `vite dev`.
- [2026-09-16] 10d (parte 2): `/normalize apps/web` — 0 Critical/High (AC-5/AC-10.2). Mediums tratados: 233 chaves i18n sem nenhuma referência (herança de outro projeto: `team.*`, `address.*`, convites, paginação, upload…) removidas dos três idiomas por um script conservador (casa literais com e sem namespace, sufixos de plural e prefixos de template dinâmico; os `.spec` contam como uso), paridade pt-BR/en/es mantida; `RecipientNumberField` ganhou o comentário da exceção F-H6. Docs sincronizados com o código: `patterns/frontend.md`, `commands/{frontend,fullstack}.md` (rotas por layout em vez de `withAppShell`), `patterns/e2e.md` (estrutura de POMs, quando `page.route` é aceito, portas do dev, `useDebounce.ts`, specs visuais), `apps/web/README.md` e `packages/theme/README.md` (novo). Orçamento do bundle inicial (`build:web`, 5 arquivos pré-carregados): 1,24 MB / 368 kB gzip, dos quais `vendor-chakra` 179 kB e o chunk de entrada 127 kB; os `manualChunks` continuam válidos. Revisitar depois do tema novo. Pendência para o usuário: portar as mudanças de `.claude/**` para o `cuidda`.
- [2026-09-16] Revisão final independente de 10a–10d: 0 Critical/High, aprovada. Medium aplicado: `patterns/frontend.md` passa a mostrar o `signal` no hook canônico (sem isso o próximo hook escrito pelo doc nasceria sem cancelamento). Nota de escopo acrescentada ao `setup-env.ts` (o fallback vale porque todo spec da API é unitário). Low aceito: `cardByName` casa por substring com `.first()` — hoje inalcançável com os nomes únicos. Observação registrada: a contagem de exclusões sobrepostas está testada com duas mutações; três ou mais dependem de uma garantia de ordem do TanStack que o teste atual não prova. CI do PR verde (build, unit e e2e da web).
- [2026-09-16] Fase 10 fechada. A Fase 0 (design) destravou em seguida: o usuário autorizou o acesso ao handoff, e a reconstrução visual continua nas Fases 3+.
